"""
Alembic env.py for transport-service.
...
transport_db_url = os.getenv("TRANSPORT_DATABASE_URL", "").replace("+asyncpg", "")
if transport_db_url:
    config.set_main_option("sqlalchemy.url", transport_db_url)
...
def include_object(object, name, type_, reflected, compare_to):
    """Only autogenerate for 'transport' schema objects."""
    if type_ == "table":
        return object.schema == "transport"
    return True
...
def run_migrations_offline() -> None:
    ...
    context.configure(
        ...
        version_table_schema="transport",
    )
...
def run_migrations_online() -> None:
    ...
    with connectable.connect() as connection:
        # Ensure the transport schema exists before running migrations
        connection.execute(text("CREATE SCHEMA IF NOT EXISTS transport"))
        connection.commit()
