...
Base.metadata.schema = "payment"
import smartsync_db.models.payment  # Ensure all payment models are imported and registered on Base.metadata
...
# 3. Set target metadata default schema to 'payment' to scope all tables
Base.metadata.schema = "payment"
target_metadata = Base.metadata


def include_object(object, name, type_, reflected, compare_to):
    """Filters objects to only include those in the 'payment' schema."""
    if type_ == "table":
        return object.schema == "payment"
    elif type_ == "schema":
        return name == "payment"
    return True
...
async def run_async_migrations() -> None:
    ...
    async with connectable.connect() as connection:
        ...
        await connection.execute(text("CREATE SCHEMA IF NOT EXISTS payment"))
