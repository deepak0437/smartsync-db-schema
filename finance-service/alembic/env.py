"""
Alembic env.py for finance-service.
...
finance_db_url = os.getenv("FINANCE_DATABASE_URL", "").replace("+asyncpg", "")
if finance_db_url:
    config.set_main_option("sqlalchemy.url", finance_db_url)
...
def include_object(object, name, type_, reflected, compare_to):
    """Only autogenerate for 'finance' schema objects."""
    if type_ == "table":
        return object.schema == "finance"
    return True
...
def run_migrations_online() -> None:
    ...
    with connectable.connect() as connection:
        # Ensure the finance schema exists before running migrations
