"""
Finance Service — Database session configuration.
...
DATABASE_URL: str = os.getenv(
    ...
    "postgresql+asyncpg://smartsync:smartsync@localhost:5432/smartsync_finance",
)
...
engine = create_async_engine(
    ...
    connect_args={
        "server_settings": {
            "search_path": "finance",          # Default schema for finance service
            "application_name": "smartsync-finance-service",
        }
    },
)
...
async def init_db() -> None:
    ...
    async with engine.begin() as conn:
        # Ensure the 'finance' schema exists
        await conn.execute(__import__("sqlalchemy").text("CREATE SCHEMA IF NOT EXISTS finance"))
