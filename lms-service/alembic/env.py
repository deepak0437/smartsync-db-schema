"""
Alembic env.py for lms-service.
...
lms_db_url = os.getenv("LMS_DATABASE_URL", "").replace("+asyncpg", "")
if lms_db_url:
    config.set_main_option("sqlalchemy.url", lms_db_url)
...
def include_object(object, name, type_, reflected, compare_to):
    """Only autogenerate for 'lms' schema objects."""
    if type_ == "table":
        return object.schema == "lms"
    return True
...
def run_migrations_online() -> None:
    ...
    with connectable.connect() as connection:
        # Ensure the lms schema exists before running migrations
