"""
LMS Service — Database session configuration.
...
DATABASE_URL: str = os.getenv(
    "LMS_DATABASE_URL",
    "postgresql+asyncpg://smartsync:smartsync@localhost:5432/smartsync_lms",
)
...
engine = create_async_engine(
    ...
    connect_args={
        "server_settings": {
            "search_path": "lms",          # Default schema for lms service
            "application_name": "smartsync-lms-service",
        }
    },
)
...
async def init_db() -> None:
    ...
    async with engine.begin() as conn:
        # Ensure the 'lms' schema exists
        await conn.execute(__import__("sqlalchemy").text("CREATE SCHEMA IF NOT EXISTS lms"))
