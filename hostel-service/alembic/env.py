"""
Alembic env.py for hostel-service.
...
hostel_db_url = os.getenv("HOSTEL_DATABASE_URL", "").replace("+asyncpg", "")
if hostel_db_url:
    config.set_main_option("sqlalchemy.url", hostel_db_url)
...
def include_object(object, name, type_, reflected, compare_to):
    """Only autogenerate for 'hostel' schema objects."""
    if type_ == "table":
        return object.schema == "hostel"
    return True
...
def run_migrations_online() -> None:
    ...
    with connectable.connect() as connection:
        # Ensure the hostel schema exists before running migrations
        connection.execute(text("CREATE SCHEMA IF NOT EXISTS hostel"))
        connection.commit()
