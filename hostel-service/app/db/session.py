"""
Hostel Service — Database session configuration.
...
DATABASE_URL: str = os.getenv(
    "HOSTEL_DATABASE_URL",
    "postgresql+asyncpg://smartsync:smartsync@localhost:5432/smartsync_hostel",
)
...
engine = create_async_engine(
    ...
    connect_args={
        "server_settings": {
            "search_path": "hostel",          # Default schema for hostel service
            "application_name": "smartsync-hostel-service",
        }
    },
)
...
async def init_db() -> None:
    ...
    async with engine.begin() as conn:
        # Ensure the 'hostel' schema exists
        await conn.execute(__import__("sqlalchemy").text("CREATE SCHEMA IF NOT EXISTS hostel"))
