"""
Alembic env.py for analytics-service.
...
analytics_db_url = os.getenv("ANALYTICS_DATABASE_URL", "").replace("+asyncpg", "")
if analytics_db_url:
    config.set_main_option("sqlalchemy.url", analytics_db_url)
...
def include_object(object, name, type_, reflected, compare_to):
    """Only autogenerate for 'analytics' schema objects."""
    if type_ == "table":
...
def run_migrations_online() -> None:
    ...
    with connectable.connect() as connection:
        # Ensure the analytics schema exists before running migrations
