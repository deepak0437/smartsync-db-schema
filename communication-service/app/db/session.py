"""
Communication Service — Database session configuration.
...
DATABASE_URL: str = os.getenv(
    ...
    "postgresql+asyncpg://smartsync:smartsync@localhost:5432/smartsync_communication",
)
...
engine = create_async_engine(
    ...
    connect_args={
        "server_settings": {
            "search_path": "communication",          # Default schema for communication service
            "application_name": "smartsync-communication-service",
        }
    },
)
...
async def init_db() -> None:
    ...
    async with engine.begin() as conn:
        # Ensure the 'communication' schema exists
        await conn.execute(__import__("sqlalchemy").text("CREATE SCHEMA IF NOT EXISTS communication"))
