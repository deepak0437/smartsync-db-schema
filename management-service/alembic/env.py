"""
Alembic env.py for management-service.
...
management_db_url = os.getenv("MANAGEMENT_DATABASE_URL", "").replace("+asyncpg", "")
if management_db_url:
    config.set_main_option("sqlalchemy.url", management_db_url)
...
def include_object(object, name, type_, reflected, compare_to):
    """Only autogenerate for 'management' schema objects."""
    if type_ == "table":
        return object.schema == "management"
    return True
...
def run_migrations_online() -> None:
    ...
    with connectable.connect() as connection:
        # Ensure the management schema exists before running migrations
        connection.execute(text("CREATE SCHEMA IF NOT EXISTS management"))
        connection.commit()
