"""
Alembic env.py for media-service.
...
media_db_url = os.getenv("MEDIA_DATABASE_URL", "").replace("+asyncpg", "")
if media_db_url:
    config.set_main_option("sqlalchemy.url", media_db_url)
...
def include_object(object, name, type_, reflected, compare_to):
    """Only autogenerate for 'media' schema objects."""
    if type_ == "table":
        return object.schema == "media"
    return True
...
def run_migrations_online() -> None:
    ...
    with connectable.connect() as connection:
        # Ensure the media schema exists before running migrations
        connection.execute(text("CREATE SCHEMA IF NOT EXISTS media"))
        connection.commit()
