"""
Media Service — Database session configuration.
...
DATABASE_URL: str = os.getenv(
    "MEDIA_DATABASE_URL",
    "postgresql+asyncpg://smartsync:smartsync@localhost:5432/smartsync_media",
)
...
engine = create_async_engine(
    ...
    connect_args={
        "server_settings": {
            "search_path": "media",          # Default schema for media service
            "application_name": "smartsync-media-service",
        }
    },
)
...
async def init_db() -> None:
    ...
    async with engine.begin() as conn:
        # Ensure the 'media' schema exists
        await conn.execute(__import__("sqlalchemy").text("CREATE SCHEMA IF NOT EXISTS media"))
