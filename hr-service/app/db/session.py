"""
HR Service — Database session configuration.
...
DATABASE_URL: str = os.getenv(
    "HR_DATABASE_URL",
    "postgresql+asyncpg://smartsync:smartsync@localhost:5432/smartsync_hr",
)
...
engine = create_async_engine(
    ...
    connect_args={
        "server_settings": {
            "search_path": "hr",          # Default schema for hr service
            "application_name": "smartsync-hr-service",
        }
    },
)
...
async def init_db() -> None:
    ...
    async with engine.begin() as conn:
        # Ensure the 'hr' schema exists
        await conn.execute(__import__("sqlalchemy").text("CREATE SCHEMA IF NOT EXISTS hr"))
