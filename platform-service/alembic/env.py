"""
Alembic env.py for platform-service.
...
platform_db_url = os.getenv("PLATFORM_DATABASE_URL", "").replace("+asyncpg", "")
if platform_db_url:
    config.set_main_option("sqlalchemy.url", platform_db_url)
...
def include_object(object, name, type_, reflected, compare_to):
    """Only autogenerate for 'platform' schema objects."""
    if type_ == "table":
...
def run_migrations_online() -> None:
    ...
    with connectable.connect() as connection:
        # Ensure the platform schema exists before running migrations
