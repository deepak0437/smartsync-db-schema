"""
Administration Service — Database session configuration.
...
DATABASE_URL: str = os.getenv(
    ...
    "postgresql+asyncpg://smartsync:smartsync@localhost:5432/smartsync_administration",
)
...
engine = create_async_engine(
    ...
    connect_args={
        "server_settings": {
            "search_path": "administration",          # Default schema for administration service
            "application_name": "smartsync-administration-service",
        }
    },
)
...
async def init_db() -> None:
    ...
    async with engine.begin() as conn:
        # Ensure the 'administration' schema exists
        await conn.execute(__import__("sqlalchemy").text("CREATE SCHEMA IF NOT EXISTS administration"))
