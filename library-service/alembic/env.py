"""
Alembic env.py for library-service.
...
library_db_url = os.getenv("LIBRARY_DATABASE_URL", "").replace("+asyncpg", "")
if library_db_url:
    config.set_main_option("sqlalchemy.url", library_db_url)
...
def include_object(object, name, type_, reflected, compare_to):
    """Only autogenerate for 'library' schema objects."""
    if type_ == "table":
        return object.schema == "library"
    return True
...
def run_migrations_online() -> None:
    ...
    with connectable.connect() as connection:
        # Ensure the library schema exists before running migrations
        connection.execute(text("CREATE SCHEMA IF NOT EXISTS library"))
        connection.commit()
