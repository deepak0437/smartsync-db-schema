"""
Alembic env.py for security-service.
...
security_db_url = os.getenv("SECURITY_DATABASE_URL", "").replace("+asyncpg", "")
if security_db_url:
    config.set_main_option("sqlalchemy.url", security_db_url)
...
def include_object(object, name, type_, reflected, compare_to):
    """Only autogenerate for 'security' schema objects."""
    if type_ == "table":
        return object.schema == "security"
    return True
...
def run_migrations_online() -> None:
    ...
    with connectable.connect() as connection:
        # Ensure the security schema exists before running migrations
