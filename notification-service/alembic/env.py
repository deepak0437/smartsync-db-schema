"""
Alembic env.py for notification-service.
...
notification_db_url = os.getenv("NOTIFICATION_DATABASE_URL", "").replace("+asyncpg", "")
if notification_db_url:
    config.set_main_option("sqlalchemy.url", notification_db_url)
...
def include_object(object, name, type_, reflected, compare_to):
    """Only autogenerate for 'notification' schema objects."""
    if type_ == "table":
        return object.schema == "notification"
    return True
...
def run_migrations_online() -> None:
    ...
    with connectable.connect() as connection:
        # Ensure the notification schema exists before running migrations
        connection.execute(text("CREATE SCHEMA IF NOT EXISTS notification"))
        connection.commit()
